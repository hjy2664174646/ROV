...
uint8_t BMI088_init(SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate)
{
    ...
    else
    {
        ...
        // Use current temperature as baseline when offsets are loaded from defines.
        float temp_sum = 0.0f;
        for (uint8_t i = 0; i < 5; i++)
        {
            temp_sum += BMI088_Read_Temperature();
            HAL_Delay(2);
        }
        BMI088.TempWhenCali = temp_sum / 5.0f;
    }
...
void Calibrate_MPU_Offset(IMU_Data_t *bmi088)
{
    ...
    printf("BMI088 calib final offsets: Gyro=%.6f %.6f %.6f | Accel=%.6f %.6f %.6f\\n",
           bmi088->GyroOffset[0],
           bmi088->GyroOffset[1],
           bmi088->GyroOffset[2],
...
uint8_t bmi088_accel_init(void)
{
    ...
    {
        printf("[BMI088 accel] chip ID mismatch: 0x%02X\n", res);
    ...
    for (write_reg_num = 0; write_reg_num < BMI088_WRITE_ACCEL_REG_NUM; write_reg_num++)
    {
        ...
        if (res != write_BMI088_accel_reg_data_error[write_reg_num][1])
        {
            ...
            printf("[BMI088 accel] reg 0x%02X expected 0x%02X got 0x%02X err=0x%02X\n",
    ...
    printf("[BMI088 accel init] result=0x%02X\n", error);
...
uint8_t bmi088_gyro_init(void)
{
    ...
    {
        printf("[BMI088 gyro] chip ID mismatch: 0x%02X\n", res);
        return BMI088_NO_SENSOR;
    }
    ...
    for (write_reg_num = 0; write_reg_num < BMI088_WRITE_GYRO_REG_NUM; write_reg_num++)
    {
        ...
        const uint8_t reg_index = write_reg_num;
        if (res != write_BMI088_gyro_reg_data_error[write_reg_num][1])
        {
            write_reg_num--;
            error |= write_BMI088_gyro_reg_data_error[reg_index][2];
            printf("[BMI088 gyro] reg 0x%02X expected 0x%02X got 0x%02X err=0x%02X\n",
                   write_BMI088_gyro_reg_data_error[reg_index][0],
                   write_BMI088_gyro_reg_data_error[reg_index][1],
                   res,
                   error);
        }
    }

    printf("[BMI088 gyro init] result=0x%02X\n", error);
    return BMI088_NO_ERROR;
}
...
void BMI088_Read(IMU_Data_t *bmi088)
{
    ...
    if (caliOffset)
    {
        float temp_delta = bmi088->Temperature - bmi088->TempWhenCali;
        bmi088->Gyro[0] -= BMI088_GYRO_TEMP_SLOPE_X * temp_delta;
        bmi088->Gyro[1] -= BMI088_GYRO_TEMP_SLOPE_Y * temp_delta;
        bmi088->Gyro[2] -= BMI088_GYRO_TEMP_SLOPE_Z * temp_delta;
    }
#endif

#if BMI088_TEMP_DEBUG
    static uint32_t temp_debug_count = 0;
    if (++temp_debug_count >= 200)
    {
        temp_debug_count = 0;
        printf("[BMI088 TEMP] T=%.2fC dT=%.2f gyro=%.6f %.6f %.6f slopes=%.6f %.6f %.6f\n",
               bmi088->Temperature,
               bmi088->Temperature - bmi088->TempWhenCali,
               bmi088->Gyro[0],
               bmi088->Gyro[1],
               bmi088->Gyro[2],
               BMI088_GYRO_TEMP_SLOPE_X,
               BMI088_GYRO_TEMP_SLOPE_Y,
               BMI088_GYRO_TEMP_SLOPE_Z);
    }
#endif
}
...
static float BMI088_Read_Temperature(void)
{
    ...
    return raw_temp * BMI088_TEMP_FACTOR + BMI088_TEMP_OFFSET;
}

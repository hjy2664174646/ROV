/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define SERVO2_PWM_Pin GPIO_PIN_5
#define SERVO2_PWM_GPIO_Port GPIOE
#define SERVO3_PWM_Pin GPIO_PIN_6
#define SERVO3_PWM_GPIO_Port GPIOE
#define ALARM_Pin GPIO_PIN_0
#define ALARM_GPIO_Port GPIOC
#define SPI1_CS_Pin GPIO_PIN_1
#define SPI1_CS_GPIO_Port GPIOC
#define Accel_INT_Pin GPIO_PIN_10
#define Accel_INT_GPIO_Port GPIOE
#define Gyro_INT_Pin GPIO_PIN_12
#define Gyro_INT_GPIO_Port GPIOE
#define HEAT_Pin GPIO_PIN_12
#define HEAT_GPIO_Port GPIOB
#define LIGHT_PWM_Pin GPIO_PIN_14
#define LIGHT_PWM_GPIO_Port GPIOD
#define SERVO1_PWM_Pin GPIO_PIN_15
#define SERVO1_PWM_GPIO_Port GPIOD
#define PPS_Pin GPIO_PIN_8
#define PPS_GPIO_Port GPIOA
#define SPI2_Accel_CS_Pin GPIO_PIN_0
#define SPI2_Accel_CS_GPIO_Port GPIOD
#define SPI2_Gyro_CS_Pin GPIO_PIN_1
#define SPI2_Gyro_CS_GPIO_Port GPIOD
#define IIC1_SCL_Pin GPIO_PIN_8
#define IIC1_SCL_GPIO_Port GPIOB
#define IIC1_SDA_Pin GPIO_PIN_9
#define IIC1_SDA_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
